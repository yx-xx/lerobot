...
def read_pose_m(robot: frankx.Robot) -> tuple[float, float, float]:
    # 不要用 read_once().O_T_EE：frankx 绑定里它可能是 16 维平铺矩阵。
    pose = robot.current_pose()
    x, y, z = (float(v) for v in pose.translation())
    if not all(_finite(v) for v in (x, y, z)):
        raise SystemExit("current_pose() 返回了非有限值")
    return x, y, z
...
def main() -> None:
    ...
    try:
        ...
        print_result(x_range, y_range, z_range, g_range)
